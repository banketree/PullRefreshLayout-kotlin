...
class ShowGravity internal constructor(private val prl: PullRefreshLayout) {
    ...
    fun layout(left: Int, top: Int, right: Int, bottom: Int) {
        if (prl.headerView != null) {
            ...
            when (headerShowGravity) {
                FOLLOW, FOLLOW_PLACEHOLDER, FOLLOW_CENTER -> prl.headerView.layout(
                    paddingLeft + lp.leftMargin,
                    top + lp.topMargin + paddingTop - prl.headerView.measuredHeight,
                    paddingLeft + lp.leftMargin + prl.headerView.measuredWidth,
                    top + lp.topMargin + paddingTop
                )
                PLACEHOLDER, PLACEHOLDER_CENTER, PLACEHOLDER_FOLLOW -> prl.headerView.layout(
                    paddingLeft + lp.leftMargin, top + paddingTop + lp.topMargin,
                    paddingLeft + lp.leftMargin + prl.headerView.measuredWidth,
                    top + paddingTop + lp.topMargin + prl.headerView.measuredHeight
                )
                CENTER, CENTER_FOLLOW -> prl.headerView.layout(
                    paddingLeft + lp.leftMargin,
                    top + paddingTop - prl.headerView.measuredHeight / 2,
                    paddingLeft + lp.leftMargin + prl.headerView.measuredWidth,
                    top + paddingTop + prl.headerView.measuredHeight / 2
                )
            }
        ...
        if (prl.footerView != null) {
            ...
            when (footerShowGravity) {
                FOLLOW, FOLLOW_PLACEHOLDER, FOLLOW_CENTER -> prl.footerView.layout(
                    lp.leftMargin + paddingLeft, bottom - lp.topMargin - paddingBottom,
                    lp.leftMargin + paddingLeft + prl.footerView.measuredWidth,
                    bottom - lp.topMargin - paddingBottom + prl.footerView.measuredHeight
                )
                PLACEHOLDER, PLACEHOLDER_CENTER, PLACEHOLDER_FOLLOW -> prl.footerView.layout(
                    lp.leftMargin + paddingLeft,
                    bottom - lp.bottomMargin - paddingBottom - prl.footerView.measuredHeight,
                    lp.leftMargin + paddingLeft + prl.footerView.measuredWidth,
                    bottom - lp.bottomMargin - paddingBottom
                )
                CENTER, CENTER_FOLLOW -> prl.footerView.layout(
                    lp.leftMargin + paddingLeft,
                    bottom - paddingBottom - prl.footerView.measuredHeight / 2,
                    lp.leftMargin + paddingLeft + prl.footerView.measuredWidth,
                    bottom - paddingBottom + prl.footerView.measuredHeight / 2
                )
            }
